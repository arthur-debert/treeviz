...
class NodeAssertion:
    ...
    def has_children_count(self, expected: int) -> "NodeAssertion":
        ...
        ), f"Expected {expected} children, got {actual_count}"
        return self

    def has_children(self, expected: List[Node]) -> "NodeAssertion":
        """Assert node has exactly these children."""
        assert (
            self.node.children == expected
        ), f"Expected children {expected}, got {self.node.children}"
        return self
