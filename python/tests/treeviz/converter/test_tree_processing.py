...
def test_single_node_conversion(assert_node):
    ...
    assert_node(result).has_label("Leaf Node").has_type("leaf").has_children_count(0)
...
def test_parent_child_conversion(assert_node):
    ...
    assert_node(result).has_label("Parent").has_type("parent").has_children_count(2)
...
def test_deep_tree_conversion(assert_node):
    ...
    assert_node(child_result).has_label("Child").has_type("branch").has_children_count(1)
    
    grandchild_result = child_result.children[0]
    assert_node(grandchild_result).has_label("Grandchild").has_type("leaf").has_children_count(0)
...
def test_children_conversion_error():
    ...
    converter = DeclarativeConverter(config)
    with pytest.raises(ConversionError, match="Children attribute must return a list"):
...
def test_recursive_tree_processing():
    ...
    root = MockNode(name="Root", node_type="root", child_nodes=[branch1, branch2])
