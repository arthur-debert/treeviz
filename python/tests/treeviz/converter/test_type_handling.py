...
def test_type_overrides_multiple_attributes(assert_node):
    ...
    assert_node(result).has_label("Correct Title").has_metadata({"special": True})
...
def test_type_overrides_multiple_types():
    ...
    # Test text type
    text_source = MockNode(name="Wrong", content="Text Content", node_type="text")
    text_result = converter.convert(text_source)
    assert text_result.label == "Text Content"
    
    # Test heading type
    heading_source = MockNode(name="Wrong", title="Heading Title", node_type="heading")
...
def test_ignore_types_nested_trees(assert_node):
    ...
    branch = MockNode(
        name="Branch", 
        node_type="branch",
        child_nodes=[comment1, text1]
    )
    
    root = MockNode(
        name="Root",
        node_type="root",
        child_nodes=[branch, comment2, text2]
    )
