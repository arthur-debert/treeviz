...
def test_default_pack_fallback():
    """Test that the default '3viz' pack is used when no mapping is provided."""
...
def test_default_pack_alias_fallback():
    """Test that an alias in the default '3viz' pack is resolved."""
...
def test_custom_default_pack():
    ...
    custom_pack = IconPack(name="custom", icons={"custom_type": Icon(icon="🎉")})
...
def test_invalid_icon_pack_name_in_adapter():
    ...
    with pytest.raises(ValueError, match="Icon pack name 'invalid.pack' is not valid."):
        adapt_node(SimpleNode("a"), {
...
def test_invalid_icon_name_in_adapter():
    ...
    with pytest.raises(ValueError, match="Icon name 'invalid-icon' is not a valid identifier."):
        adapt_node(SimpleNode("a"), {
            "ICON_PACKS": [
                {
                    "name": "user_pack",
...
def test_mixed_mode_icons():
    ...
    assert adapt_node(SimpleNode("heading"), def_).icon == "⊤" # From 3viz pack
    assert adapt_node(SimpleNode("para"), def_).icon == "¶" # Alias from 3viz pack